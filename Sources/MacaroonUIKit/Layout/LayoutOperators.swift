...
extension ConstraintMakerExtendable {
    ...
    public static func == (
        ...
        switch rhs.target {
        case .superview: maker = lhs.equalToSuperview()
        case .constraintItem(let constraintItem): maker = lhs.equalTo(constraintItem)
        case .view(let view): maker = lhs.equalTo(view)
        }
    ...
    public static func >= (
        ...
        switch rhs.target {
        case .superview: maker = lhs.greaterThanOrEqualToSuperview()
        case .constraintItem(let constraintItem): maker = lhs.greaterThanOrEqualTo(constraintItem)
        case .view(let view): maker = lhs.greaterThanOrEqualTo(view)
        }
    ...
    public static func <= (
        ...
        switch rhs.target {
        case .superview: maker = lhs.lessThanOrEqualToSuperview()
        case .constraintItem(let constraintItem): maker = lhs.lessThanOrEqualTo(constraintItem)
        case .view(let view): maker = lhs.lessThanOrEqualTo(view)
        }
    ...
    private static func makeConstraint(
        ...
        if let multiplier = layoutConstraint.multiplier {
            maker = maker.multipliedBy(multiplier)
        }

        if let constant = layoutConstraint.constant {
            if layoutConstraint.isRelational {
                maker = maker.offset(constant)
            } else {
                maker = maker.inset(constant)
            }
        }
