...
open class CameraController:
    ...
    open func observeNotifications() {
        observe(
            notificationWith: .AVCaptureSessionDidStartRunning,
            onNotified: sessionDidStartRunning
        )
        observe(
            notificationWith: .AVCaptureSessionDidStopRunning,
            onNotified: sessionDidStopRunning
        )
        observe(
            notificationWith: .AVCaptureSessionRuntimeError,
            onNotified: sessionDidFailOnRuntime
        )
        observe(
            notificationWith: .AVCaptureSessionWasInterrupted,
            onNotified: sessionWasInterrupted
        )
        observe(
            notificationWith: .AVCaptureSessionInterruptionEnded,
            onNotified: sessionDidReturnFromInterruption
        )
    }
    ...
    open func sessionDidStartRunning(
        ...
        asyncMain(
            ...
            strongSelf.delegate?.cameraControllerDidStartRunning(
                strongSelf
            )
        }
    }
    ...
    open func sessionDidStopRunning(
        ...
        asyncMain(
            ...
            strongSelf.delegate?.cameraControllerDidStopRunning(
                strongSelf
            )
        }
    }
    ...
    open func sessionDidReturnFromInterruption(
        ...
        asyncMain(
            ...
            strongSelf.delegate?.cameraControllerDidReturnFromInterruption(
                strongSelf
            )
        }
    }
        ...
        sessionQueue.async {
            ...
            if self.session.isRunning {
                asyncMain(
                    ...
                    strongSelf.delegate?.cameraControllerDidResumeRunningAfterInterruption(
                        strongSelf
                    )
                }
            } else {
...
public protocol CameraControllerDelegate: AnyObject {
    func cameraControllerDidStartRunning(_ cameraController: CameraController)
    func cameraControllerDidResumeRunningAfterInterruption(_ cameraController: CameraController)
    func cameraControllerDidStopRunning(_ cameraController: CameraController)
    func cameraController(_ cameraController: CameraController, wasInterrupted reason: AVCaptureSession.InterruptionReason)
    func cameraControllerDidReturnFromInterruption(_ cameraController: CameraController)
    func cameraController(_ cameraController: CameraController, didMalfunction error: CameraController.Error)
}
